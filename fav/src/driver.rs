...
mod lineage_tests {
    ...
    // D-1: extract_tables_from_sql — FROM
    #[test]
    fn extract_tables_from_sql_basic() {
        let (reads, writes) = extract_tables_from_sql("SELECT * FROM orders WHERE id = 1");
        assert!(reads.contains(&"ORDERS".to_string()));
    ...
    fn extract_tables_join() {
        let (reads, _writes) = extract_tables_from_sql(
            "SELECT o.id, u.name FROM orders o JOIN users u ON o.user_id = u.id",
        );
        assert!(reads.contains(&"ORDERS".to_string()));
        assert!(reads.contains(&"USERS".to_string()));
    }
    ...
    fn extract_tables_insert() {
        let (_reads, writes) =
            extract_tables_from_sql("INSERT INTO shipments (order_id, status) VALUES (1, 'sent')");
        assert!(writes.contains(&"SHIPMENTS".to_string()));
    }

    // D-1: UPDATE
    #[test]
    fn extract_tables_update() {
        let (_reads, writes) =
            extract_tables_from_sql("UPDATE orders SET status = 'shipped' WHERE id = 1");
        assert!(writes.contains(&"ORDERS".to_string()));
    }

    // D-1: DELETE FROM
    #[test]
    fn extract_tables_delete() {
        let (_reads, writes) = extract_tables_from_sql("DELETE FROM old_logs WHERE ts < 1000");
        assert!(writes.contains(&"OLD_LOGS".to_string()));
    }
...
"#;
        ...
        assert_eq!(report.pipelines[0].name, "Pipeline");
        assert_eq!(report.pipelines[0].steps, vec!["ReadOrders", "WriteShipments"]);
    }

    // D-3: render_lineage_text smoke test
    #[test]
...
"#;
        ...
        assert!(text.contains("Sources:"), "missing Sources section");
        assert!(text.contains("Sinks:"), "missing Sinks section");
        assert!(text.contains("Transformations:"), "missing Transformations section");
        assert!(text.contains("Pipelines:"), "missing Pipelines section");
        assert!(text.contains("ReadOrders"));
...
// ── pyproject.toml / README.md 生成ヘルパー (v11.7.0) ────────────────────────

pub fn build_pyproject_content(py_src: &str, name: &str) -> String {
...
pub fn build_readme_content(input_path: &str, name: &str) -> String {
    format!(
        "# {name}\n\n\
...
pub fn cmd_transpile(args: &[String]) {
    ...
    if let Some(ref dir) = out_dir {
        ...
        // main.py
        let main_py = dir_path.join("main.py");
        if let Err(e) = std::fs::write(&main_py, &py_src) {
            eprintln!("error writing main.py: {}", e);
            std::process::exit(1);
        }
        println!("transpiled: {} -> {}/main.py", input, dir);
        final_out = main_py.to_str().unwrap_or("main.py").to_string();

        // pyproject.toml（既存スキップ）
    ...
    if do_check {
        let status = std::process::Command::new("python")
            .args(["-m", "py_compile", &final_out])
            .status();
        match status {
            Ok(s) if s.success() => {
                println!("fav transpile: syntax ok ({})", final_out);
            }
            Ok(_) => {
                eprintln!("error: python syntax check failed: {}", final_out);
                std::process::exit(1);
            }
            Err(_) => {
...
// ── v11700_tests (v11.7.0) — uv 統合: --out-dir / --check / --run ────────────

#[cfg(test)]
mod v11700_tests {
    use crate::driver::{build_pyproject_content, build_readme_content};
    use crate::emit_python::emit_python_str;
    use std::fs;
    use tempfile::TempDir;
    ...
    fn run_transpile(args: &[&str]) {
        let str_args: Vec<String> = args.iter().map(|s| s.to_string()).collect();
        crate::driver::cmd_transpile(&str_args);
    }

    #[test]
    fn transpile_out_dir_creates_main_py() {
        let tmp = TempDir::new().unwrap();
        let fav_path = write_fav(&tmp, "hello.fav", "fn greet(name: String) -> String {\n  name\n}");
        ...
        assert!(out_dir.join("main.py").exists(), "main.py should exist");
        let py = fs::read_to_string(out_dir.join("main.py")).unwrap();
        assert!(py.contains("def greet("), "greet fn:\n{}", py);
    }

    #[test]
    fn transpile_out_dir_creates_pyproject() {
        let tmp = TempDir::new().unwrap();
        let fav_path = write_fav(&tmp, "app.fav", "fn run() -> String {\n  \"ok\"\n}");
        let out_dir = tmp.path().join("out");
        run_transpile(&[
            "--target", "python",
            &fav_path,
    ...
    #[test]
    fn transpile_out_dir_creates_readme() {
        let tmp = TempDir::new().unwrap();
        let fav_path = write_fav(&tmp, "app.fav", "fn run() -> String {\n  \"ok\"\n}");
        let out_dir = tmp.path().join("out");
        run_transpile(&[
        ...
        assert!(out_dir.join("README.md").exists(), "README.md should exist");
        let readme = fs::read_to_string(out_dir.join("README.md")).unwrap();
        assert!(readme.contains("uv run main.py"), "readme content:\n{}", readme);
    }

    #[test]
    fn transpile_check_valid_python() {
        // --check フラグは python がなければ warning のみ（CI 環境でも安全）
        // emit_python_str で valid Python が生成されることを確認
        let src = "fn add(a: Int, b: Int) -> Int {\n  a\n}";
        let out = emit_python_str(src);
        // 生成コードに Python の構文的に壊れた要素がないことを基本チェック
        assert!(out.contains("def add("), "add fn:\n{}", out);
        assert!(!out.contains("syntax error"), "no syntax error text:\n{}", out);
    }

    #[test]
    fn transpile_pyproject_project_name() {
        let py_src = "# generated";
        let content = build_pyproject_content(py_src, "my_pipeline");
        assert!(content.contains("name = \"my_pipeline\""), "project name:\n{}", content);
    }

    #[test]
    fn transpile_pyproject_has_correct_deps() {
        // boto3 + psycopg2 両方含む場合
        let py_src = "import boto3\nimport psycopg2\n# code";
        let content = build_pyproject_content(py_src, "demo");
        assert!(content.contains("boto3>=1.34"), "boto3 dep:\n{}", content);
        assert!(content.contains("psycopg2-binary>=2.9"), "psycopg2 dep:\n{}", content);

        // どちらも含まない場合
        let py_empty = "print('hello')";
