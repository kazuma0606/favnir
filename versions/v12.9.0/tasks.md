...
- [ ] A-1: `compiler.fav` の `fav test` 動作を手元で確認
  - `./target/debug/fav test self/compiler.fav` を実行
  - 失敗する場合はそのファイルのみスキップし理由をコメントに記載
- [ ] A-2: `.github/workflows/ci.yml` の `Self-fmt` ステップ後に `Self-test` ステップを追加
  ```yaml
...
- [ ] B-1: `ci.yml` の `site` ジョブの前に `integration` ジョブを追加
...
- [ ] B-2: `integration` ジョブに以下のステップを追加
  - `actions/checkout@v4`
...
- [ ] C-1: `fav/tests/integration.rs` を新規作成
  - `db_url()` ヘルパー: `DATABASE_URL` が未設定なら `None`
  - `DATABASE_URL` なしの場合 `return;` でスキップ
- [ ] C-2: `postgres_create_insert_select` テストを実装
  ...
  - `DROP TABLE ...`（クリーンアップ）
- [ ] C-3: `postgres_error_table_not_found` テストを実装
  - 存在しないテーブルへの SELECT が `Err(...)` を返すこと
  - エラーメッセージに `"does not exist"` が含まれること
- [ ] C-4: `postgres_ssl_disable_connects` テストを実装
  - `sslmode=disable` で接続が成功すること
  - `SELECT 1` が `Ok(...)` を返すこと
- [ ] C-5: `fav_core::backend::vm` の Postgres 関数を統合テストから呼べるよう公開
...
- [ ] D-1: `run_self_test(file: &str) -> std::process::ExitStatus` ヘルパーを実装
  - `std::process::Command::new(env!("CARGO_BIN_EXE_fav"))` を使用
  - working_directory を `fav/` に設定
- [ ] D-2: 以下のテストを `v12900_tests` モジュールに追加
...
- [ ] E-1: `fav/Cargo.toml` version → `"12.9.0"`
- [ ] E-2: `fav/src/driver.rs` の `version_is_12_8_0` を comment out（次バージョンテストに委譲）
- [ ] E-3: `cargo test` — 全通過（統合テストは DATABASE_URL なしでスキップ）
- [ ] E-4: `git commit -m "feat: v12.9.0 — CI fav test self/*.fav + Postgres integration tests"`
- [ ] E-5: `git push` → CI 通過確認（`Self-test` と `integration` ジョブが緑になること）
...
| `cargo test` 全通過（統合テストは DATABASE_URL なしでスキップ） | |
| CI が全 green になる | |
