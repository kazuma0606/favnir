...
## v8.0.0 ✓ — checker.fav Let 多相（型スキーム generalization / instantiation）

**テーマ**: checker.fav に Let ポリモーフィズムを実装する。
ユーザー定義ジェネリクス関数の呼び出しを `infer_hm` が正しく推論できるようにする。

### 実装済み内容（2026-05-28 完了）

- `is_type_var_extended`: 2 文字型変数（`T0`〜`Z9`）サポート
- 型スキーム文字列形式: `"forall|A,B|List<A>;Fn|List<B>"` による直列化
- `collect_type_vars_from_te / params`: TypeExpr から型変数を再帰収集
- `fn_to_scheme_str / collect_fn_schemes`: FnDef → スキーム文字列、pre-pass で全関数を事前登録
- `build_scheme_subst_inner / apply_scheme_subst`: `unify_deep` を利用したスキーム具体化
- `instantiate_fn_scheme / infer_call_hm`: ユーザー定義ジェネリクス呼び出し推論
- `infer_hm` の `ECall` ケース追加、`check` に pre-pass 追加
- Favnir 内テスト 10 件 + driver.rs 統合テスト 3 件（計 1103 tests passing）

### 残った制約（v8.1.0 以降へ持ち越し）

- `checker.fav` はまだ `fav check` の実処理に接続されていない（v8.1.0 で対応）
- ネスト 2 段以上の型変数置換は `String.replace` の制限内（実用上問題なし）

---

## v8.1.0 — fav check 配線（checker.fav を fav check パイプラインに接続）

**テーマ**: `fav check foo.fav` が checker.rs（Rust）ではなく checker.fav（Favnir）経由で
動作するように差し替える。型チェッカーのセルフホスト完成。

### 背景

```
現状:  fav check foo.fav → checker.rs (Rust)
v8.1: fav check foo.fav → checker.fav (Favnir、Rust VM の上で動く)
```

checker.fav は v7.7.0〜v8.0.0 で機能的に checker.rs と同等に達した。
`Compiler.check_raw` プリミティブ（v7.6.0 追加）と `cli.fav` も既にある。
残る作業は Rust 側のパイプライン切り替えのみ。

### やること

**A. fav check コマンドの差し替え**

- `main.rs` の `fav check` パスを、checker.fav をロードして VM 上で実行する実装に切り替え
- 方式: `fav check foo.fav` → `checker.fav` を読み込み → `check(parse(foo.fav))` を VM 実行
- `Compiler.check_raw` の実装を checker.fav ベースに更新

**B. エラー形式の統一**

- checker.rs は `CheckError { message, span }` を返す
- checker.fav は `"E0xxx: ..."` 形式の文字列を返す
- 変換レイヤーを追加して既存 CLI 出力と互換を保つ
...
- 互換が確認できたら checker.rs を fallback/legacy として残すか削除するか決定

### 完了条件

- `fav check foo.fav` が checker.fav（Favnir 実装）経由で動作する
- `fav check fav/self/checker.fav` が checker.fav 自身でチェック通過
- 既存テストが全件通る（1103+）

---

## v8.2.0 — stdlib Favnir 化（List / String / Map 高レベル関数のセルフホスト）

**テーマ**: `vm.rs` にハードコードされている高レベル標準ライブラリ関数を Favnir で実装し、
Rust への依存を VM core（バイトコード実行・primitive I/O）のみに絞る。

### 背景

現状、`List.map` / `List.fold` / `List.filter` / `List.sort` 等は vm.rs 内に Rust で実装されている。
`runes/stdlib/list.fav` と `runes/stdlib/map.fav` はその先行実装として既に存在するが、
Rune として外部に置かれており、ビルティンを置き換えていない。

```
現状:  List.map(xs, f) → vm.rs の Rust 実装
v8.2: List.map(xs, f) → fav/self/stdlib/list.fav の Favnir 実装（VM core が呼ぶ）
```
...
- `fav/self/stdlib/list.fav` — List 高レベル関数
- `fav/self/stdlib/string.fav` — String 高レベル関数
- `fav/self/stdlib/map.fav` — Map 高レベル関数

各ファイルは `runes/stdlib/` の実装を参考にしつつ、
ビルティンとして VM から呼べる形に整備する。

**B. 移行対象関数の選定**

VM core（Rust に残す primitive）と移行対象を分ける:

| 残す（Rust / primitive） | 移行（Favnir）|
|--------------------------|--------------|
| `List.push / first / drop / empty / length / singleton` | `List.map / fold / filter / partition / sort / group_by` |
| `String.concat / length / slice / split / replace` | `String.join / trim` |
| `Map.empty / get / set / keys` | `Map.merge / count_by / from_entries` |

**C. ビルティン登録の差し替え**

- 移行した関数は vm.rs のビルティンマッチから削除
- Favnir 実装を自動ロードして置き換える仕組みを追加

### 完了条件

- `List.map` / `List.fold` / `List.filter` 等が Favnir 実装で動作する
- 既存テストが全件通る（1103+）
- Rust に残るのは VM core + primitive I/O + primitive 操作のみ
...
- **VM core のセルフホスト**: 実装コスト・メモリ安全性の観点から行わない（Rust に留める）
...
| v8.0.0 ✓ | checker.fav — Let 多相（型スキーム generalization / instantiation） | セルフホスト |
| **v8.1.0** | **fav check 配線 — checker.fav を fav check パイプラインに接続** | **型チェッカー完全セルフホスト** |
| v8.2.0 | stdlib Favnir 化（List / String / Map 高レベル関数） | ランタイムセルフホスト |
