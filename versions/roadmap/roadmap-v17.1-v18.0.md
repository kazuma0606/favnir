...
- v17.4: `let` バインディング誤実装の除去（`bind x <- expr` に統一）
...
| バインディング統一 | `bind x <- expr` で Result / 非 Result どちらも束縛可能。`let` キーワードは採用しない |
...
fn top_n<T with Scored with Ord>(items: List<T>, n: Int) -> List<T> {
  bind ranked <- List.sort_by_desc(items, |x| x.score())
  List.take(ranked, n)
}
```
...
// 変換 + フィルタ
bind valid_emails <- [String.trim(s) | s <- raw_emails, String.contains(s, "@")]

// 複数ソース（直積）
bind pairs <- [Pair(a, b) | a <- as, b <- bs]

// ネストしたリストの平坦化
bind flat <- [item | row <- matrix, item <- row]
...
// After（v17.3 以降）
bind valid_names <- [String.trim(r.name) | r <- rows, String.length(String.trim(r.name)) > 0]
```
...
### v17.4.0 — `let` バインディング除去（誤実装の修正）

**テーマ**: v17.4.0 で誤って追加した `let` キーワードを除去し、`bind x <- expr` に統一する。

**背景:**

`bind x <- expr` はもともと **Result / 非 Result どちらの値でも**使えるバインディングである。
`bind trimmed <- String.trim(row.name)` のように非 Result 値にも使えるため、
`let` キーワードは不要であり、言語の一貫性を損なう。

誤って `let` を追加してしまったため、v17.4.0 でこれを除去する。

**修正後の正しい書き方:**

```fav
// 修正前（v17.4.0 で誤って追加した let を使った例）
fn process(row: Row) -> Result<Output, String> {
  let trimmed = String.trim(row.name)   // ← 誤実装の let
  let score   = compute_score(row)      // ← 誤実装の let
  bind result <- validate({ ...row, name: trimmed, score: score })
  Result.ok(result)
}

// 修正後（bind で統一 — もともとこれが正しい）
fn process(row: Row) -> Result<Output, String> {
...
- `fav/src/frontend/lexer.rs`: `TokenKind::Let` 削除
- `fav/src/ast.rs`: `Stmt::Let { name, expr, span }` 削除
- `fav/src/frontend/parser.rs`: `parse_let_stmt` 削除
- `fav/src/middle/checker.rs`: `Stmt::Let` マッチ + E0326 削除
- `fav/src/middle/compiler.rs`: `Stmt::Let` コンパイルパス削除
- `fav/src/fmt.rs` / `emit_python.rs` / `lineage.rs` / `lint.rs` / `driver.rs`: exhaustive match から `Stmt::Let` 削除

- テスト: `v174000_tests`（5件）:
  - `version_is_17_4_0`
  - `bind_non_result_basic`（`bind x <- 42` が動作）
  - `bind_non_result_string`（`bind name <- String.trim(s)` が動作）
  - `bind_mix_result_and_non_result`（Result と非 Result の bind 混在）
  - `let_keyword_not_recognized`（`let x = 42` が parse error になる）

**完了条件（PASS=5）:**
1. `TokenKind::Let` が lexer から削除されている
2. `Stmt::Let` が ast.rs から削除されている
3. `bind x <- non_result_expr` が正しく動作する（既存動作の確認）
4. `let x = expr` が parse error になる
...
| `bind x <- non_result_expr` で非 Result 値を束縛できる（既存動作） | [ ] |
| REPL タブ補完・`:doc` が動作する | [ ] |
| `fav bench` が動作する | [ ] |
...
v17.0.0（Language Ergonomics）✅
    ...
    |                               |
    v17.4.0（let 除去修正）← 内包表記との組み合わせ確認も兼ねる
    |
...
- **`bind` の統一**: `bind x <- expr` は Result / 非 Result どちらの値でも使える。`let` キーワードは採用しない（v17.4.0 で除去済み）。
