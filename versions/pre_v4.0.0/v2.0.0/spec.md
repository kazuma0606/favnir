...
- [x] `trf F: Int -> Int = |x| x` が E2001 を発してコンパイルエラーになる
- [x] `flw P = F` が E2002 を発してコンパイルエラーになる
- [x] `cap Eq<T> = { ... }` が E2003 を発してコンパイルエラーになる
- [x] `abstract trf F: Int -> Int` が E2001 を発する
- [x] `stage`/`seq`/`interface` で書かれたコードが正常にコンパイル・実行される
- [x] `fav migrate trf_example.fav` が `stage` 記法に変換する
- [x] `fav migrate flw_example.fav --in-place` がファイルを書き換える
- [x] `examples/selfhost/lexer.fav` が `fav run` で動く
- [x] `examples/selfhost/lexer.test.fav` の全テストが `fav test` で通る
- [x] v1.9.0 の全テスト（523）から `trf`/`flw`/`cap` 使用テストを `stage`/`seq`/`interface` に書き換えた上で全て通る
- [x] `cargo build` で警告ゼロ
