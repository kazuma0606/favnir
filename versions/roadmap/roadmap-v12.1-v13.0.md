...
### v12.9.0 — CI 強化（`fav test` 追加 + docker-compose Postgres）
**テーマ**: ECS に上げる前に検証できるテスト層を CI に追加する

**背景**:
今回の fav2py E2E では「ECS に上げてみて初めてわかる」バグが多く、
デバッグサイクルが長かった。

また、`fav test` コマンドは v9.10.0 以前から実装済みで、
`self/` 下に 81 件のテストブロックが存在する（checker.fav: 47、compiler.fav: 12、
codegen.fav: 8、lexer.fav: 8、parser.fav: 6）にもかかわらず、
CI の self-check ステップでは `fav check / fav lint / fav fmt --check` しか
走っておらず **`fav test self/*.fav` が CI で実行されていない**。

Rust 側の `cargo test` がこれらを間接的にカバーしているため実害は小さいが、
Fav レベルでのテスト実行が CI から抜けているのは構造的なギャップ。

**実装 — `fav test` を CI に追加**:
- CI の self-check ステップに追加:
  ```yaml
  ...
  ```
- これにより Fav レベルのリグレッションを Rust テストとは独立して検出できる

**実装 — docker-compose Postgres 統合テスト**:
...
- `fav/tests/integration/postgres_rune_test.rs`:
  ...
  - SSL なし接続のスモークテスト
  - JSON 直接埋め込みパラメータのテスト
- テストは `#[cfg(feature = "integration")]` で分離
- テスト: `fav_test_self_checker_passes` / `postgres_create_insert_select` / `postgres_error_table_not_found`

---

### v12.10.0 — 全エラーメッセージに `help:` + `fav check --strict` + CI clippy-D-warnings 相当
**テーマ**: コンパイラの全出力を「次の行動が明示される」形式に統一し、CI の品質ゲートを Rust と同水準に揃える
...
現状 Rust CI では `cargo clippy -- -D warnings` で**警告ゼロを強制**しているが、
Fav CI では `fav lint` の警告が出ても exit 0 のまま通過する（`-D warnings` 相当がない）。
これが「Rust CI は細かい警告で止まるが Fav CI はほぼ通る」という非対称性の主因。

**実装**:
- E0001〜E0018 / W001〜W007 の全メッセージに `help:` suggestion を追加
- `fav check --strict`: W001〜W007 すべてをエラーとして扱い、exit 1（`-D warnings` 相当）
- `fav lint --deny-warnings`: lint 警告を exit 1 に昇格（CI 用フラグ）
- `fav.toml` の `[lint]` セクション拡充:
  ```toml
  [lint]
  warn_as_error = ["W006", "W007"]   # 特定警告のみエラー化
  allow = ["W004"]                    # 特定警告を抑制
  ```
- CI の self-lint ステップを `--deny-warnings` 付きに更新:
  ```yaml
...
- **pre-commit hook 更新**: `cargo clippy` に加え `fav lint --deny-warnings self/*.fav` も実行
- テスト: `all_errors_have_help` / `strict_mode_w006_is_error` / `deny_warnings_exits_1` / `strict_mode_allow_overrides`
...
| `fav test self/*.fav` を CI に追加 + CI Postgres 統合テスト | v12.9.0 |
| 全エラーに `help:` + `fav check --strict` + `fav lint --deny-warnings` | v12.10.0 |
