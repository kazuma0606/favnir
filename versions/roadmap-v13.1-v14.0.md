...
### v13.6.0 — 既存 Rune の ctx ベース版並行提供 + E2E デモ更新
**テーマ**: 旧エフェクト API と新 capability API を並行運用し、移行パスを確立する

**背景**:
既存の `self/compiler.fav` / `self/checker.fav` や
`infra/e2e-demo/` は旧 `Postgres.*` / `AWS.*` / `IO.*` 直接呼び出しで実装されている。
W009 deprecated 警告が出るだけで動作は続くが、
新設計の実証として E2E デモ（fav2py / airgap）を ctx ベースに書き換える。

**実装**:
- `infra/e2e-demo/fav2py/src/pipeline.fav` を capability-context 版に書き換え:
  ```
  // 旧
  stage LoadAndInsert: String -> Int !IO !Postgres = |path| { ... }
  // 新
  stage LoadAndInsert: LoadCtx -> String -> Int = |ctx, path| { ... }
  ```
- `infra/e2e-demo/airgap/src/pipeline.fav` 同様
- E2E デモ用 `AppCtx` の `Ctx.build` を `run.sh` から呼び出す形に更新
- `infra/e2e-demo/` の Docker + Terraform は変更不要（I/F は同じ）
- E2E 再実行 PASS=5 確認
- `fav/src/driver.rs` の `cmd_run` で ctx ベース pipeline を `--legacy` なしで実行できることを確認
- W009 件数が E2E デモから 0 になることを CI でチェック:
  ```yaml
  ./target/debug/fav check --strict infra/e2e-demo/fav2py/src/pipeline.fav
  ./target/debug/fav check --strict infra/e2e-demo/airgap/src/pipeline.fav
  ```
- テスト: `e2e_fav2py_ctx_based_compiles` / `e2e_airgap_ctx_based_compiles` / `w009_count_zero_after_migration`
...
| E2E デモ（fav2py / airgap）の ctx ベース移行 PASS=5 | v13.6.0 |
...
🟠 v13.5〜13.6   AppCtx + Ctx Rune + E2E デモ移行
                  ← 旧 API から新 API への実証。W009 ゼロを確認
