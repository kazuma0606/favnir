...
- [x] A-1: `AzureTomlConfig` 構造体を追加（`SnowflakeTomlConfig` の直後、toml.rs:96 付近）
  ```rust
  #[derive(Debug, Clone)]
...
- [x] A-2: `FavToml` 構造体に `azure: Option<AzureTomlConfig>` フィールド追加
  （`context: Option<ContextConfig>` の直後に追加）

- [x] A-3: `parse_fav_toml` に `[azure]` パースブロック追加
...
- [x] C-1: `builtin_ret_ty` の `("Ctx", ...)` ブロックに追加
  ```rust
...
- [x] C-2: `ns_env_def` の `"Ctx"` ブロックに 3 関数を追加
...
- [x] D-1: `fav/runes/ctx/crosscloud.fav` を新規作成
  ```fav
  type AwsCtx(String)
...
- [x] D-2: `target/debug/fav.exe check fav/runes/ctx/crosscloud.fav` でエラーなし確認

---

## Phase E — `fav/src/driver.rs`: inject_azure_config + バージョンバンプ

- [x] E-1: `inject_azure_config(cfg: &AzureTomlConfig)` 関数追加
...
- [x] E-2: `load_run_config` / `load_check_config` から `inject_azure_config` を呼び出す
  - `inject_snowflake_config` 呼び出しと同じブロック内に追加

- [x] E-3: `fav/Cargo.toml` バージョンを `"14.2.0"` にバンプ

- [x] E-4: `cargo build` でコンパイルエラーなし確認

---

## Phase F — `fav/src/driver.rs`: v142000_tests 追加

- [x] F-1: `v142000_tests` モジュールを追加（v141000_tests の直後推奨）
  - [x] `version_is_14_2_0` — `CARGO_PKG_VERSION == "14.2.0"` 確認
  - [x] `fav_toml_azure_section_parsed` — `parse_fav_toml` で `[azure]` をパースできる確認
  - [x] `aws_ctx_build_raw_registered` — `Ctx.build_aws_raw` で E0007 が出ない確認
  - [x] `azure_ctx_build_raw_registered` — `Ctx.build_azure_raw` で E0007 が出ない確認

  テスト本文は `plan.md` の Phase F を参照。

- [x] F-2: `cargo test v142000` で 4 件全パス確認

---

## Phase G — 全テスト + コミット

- [x] G-1: `cargo test v142000` 全 4 件パス
- [x] G-2: `cargo test` 全件パス（リグレッションなし）
- [x] G-3: `git commit -m "feat: v14.2.0 — AzureCtx / AwsCtx + fav.toml [azure]"`
