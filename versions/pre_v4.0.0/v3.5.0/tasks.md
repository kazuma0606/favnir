...
- [x] `fav/Cargo.toml`: `version = "3.5.0"`
- [x] `cargo build` 成功、`env!("CARGO_PKG_VERSION")` 伝播
- [x] `fav --version` で `favnir 3.5.0` を確認

## Phase 1: 型登録 + namespace

- [x] `fav/src/middle/checker.rs`: `"GenProfile"` を stdlib 型として登録
  - フィールド: `total: Int`, `valid: Int`, `invalid: Int`, `rate: Float`
- [x] `fav/src/middle/checker.rs`: `"Gen"` namespace を stdlib グローバル登録ループに追加
- [x] `fav/src/middle/compiler.rs`: `"Gen"` を2箇所の登録ループに追加
- [x] `fav/src/middle/checker.rs`: `("Random", "seed")` の型シグネチャを登録 → `Unit`
- [x] `fav/src/middle/checker.rs`: `Gen.*` 5関数の型シグネチャを登録
...
- [x] `thread_local! { static SEEDED_RNG: RefCell<Option<rand::rngs::SmallRng>> }` を追加
- [x] `fn random_int_impl(min: i64, max: i64) -> i64` ヘルパー追加（SEEDED_RNG を優先使用）
- [x] `fn random_float_impl() -> f64` ヘルパー追加（同上）
- [x] 既存の `"Random.int"` ビルトイン → `random_int_impl` を使うよう修正
- [x] 既存の `"Random.float"` ビルトイン → `random_float_impl` を使うよう修正

### 2-B: `Random.seed` (`backend/vm.rs`)

- [x] `"Random.seed"` ビルトイン追加
  - `SEEDED_RNG` に `SmallRng::seed_from_u64(n)` をセット
  - → `VMValue::Unit`

### 2-C: `Gen.string_val` (`backend/vm.rs`)

- [x] `fn random_alphanumeric_string(len: usize) -> String` ヘルパー追加
  - SEEDED_RNG がある場合はそれを使う; なければ `thread_rng`
  - `b"abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"` から選択
- [x] `"Gen.string_val"` ビルトイン追加 → `random_alphanumeric_string(len)`

### 2-D: `Gen.one_raw` (`backend/vm.rs`)

- [x] `fn gen_value_for_type(ty: &str) -> Result<String, String>` ヘルパー追加
...
- [x] `fn is_valid_for_type(val: &str, ty: &str) -> bool` ヘルパー追加
  ...
  - その他 → `true`
- [x] `fn is_valid_row(row: &HashMap<String, String>, meta: &TypeMeta) -> bool` 追加
- [x] `"Gen.profile_raw"` ビルトイン追加
...
- [x] Test: `random_seed_makes_deterministic`
- [x] Test: `gen_string_val_returns_correct_length`
- [x] Test: `gen_one_raw_returns_map_with_fields`
- [x] Test: `gen_list_raw_returns_n_rows`
- [x] Test: `gen_list_raw_seed_deterministic`
- [x] Test: `gen_simulate_raw_introduces_noise`
- [x] Test: `gen_profile_raw_all_valid`
- [x] Test: `gen_profile_raw_with_corrupt_data`

## Phase 3: `runes/gen/gen.fav`

- [x] `runes/gen/gen.fav` 作成（8 public 関数、全て Favnir 実装）
...
- [x] `fav check runes/gen/gen.fav` でエラーなし確認

## Phase 4: `runes/gen/gen.test.fav`

- [x] `runes/gen/gen.test.fav` 作成（10 テスト）
...
- [x] `fav/src/driver.rs` の `migrate_tests` モジュールに統合テスト追加
...
- [x] `fav/src/main.rs`: `check` アームに `--sample <n>` フラグ追加
  - `sample: Option<usize>` を `cmd_check` に渡す
- [x] `fav/src/driver.rs`: `pub fn cmd_check(file, no_warn, sample: Option<usize>)` に拡張
  - `sample.is_some()` のとき:
    1. 通常の型チェックを実行
    ...
    5. パイプラインを生成データで試し実行
    6. pass/fail を出力
- [x] `fav/src/driver.rs`: `fn collect_pipeline_input_type(program: &ast::Program) -> Option<String>` 追加
  - 最初の `stage` 定義の入力引数型を返す
  - 見つからなければ `None`
- [x] `fav/src/main.rs`: HELP テキストに `--sample N` オプションを追記
- [x] Test: `check_sample_runs_with_synthetic_data`
- [x] Test: `check_sample_skips_when_type_not_found`

## Phase 7: サンプル + ドキュメント

### サンプル

- [x] `fav/examples/gen_demo/fav.toml` 作成
- [x] `fav/examples/gen_demo/src/main.fav` 作成
  - `gen.list("User", 5, 42)` で生成 + `gen.simulate` + `gen.profile` を組み合わせ

### ドキュメント

- [x] `versions/v3.5.0/langspec.md` 作成
- [x] `versions/v3.5.0/migration-guide.md` 作成（破壊的変更なし）
- [x] `versions/v3.5.0/progress.md` を全 Phase `[x]` に更新
