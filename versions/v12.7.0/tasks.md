...
- [ ] A-1: `BuiltinPrimitive` 構造体を driver.rs に定義（namespace / name / signature / effects / returns_result / description）
- [ ] A-2: `builtin_primitives() -> Vec<BuiltinPrimitive>` を実装
...
- [ ] B-1: `render_builtins_markdown(primitives: &[BuiltinPrimitive]) -> String` を実装
  - namespace ごとに `## Namespace` ヘッダ
  - 各関数に `### Name` + シグネチャ + description
- [ ] B-2: JSON 出力パス（`serde_json::to_string_pretty`）を実装
...
- [ ] C-1: `get_explain_text(code: &str) -> Option<&'static str>` を実装
  - E0001〜E0018 の説明文（コード + タイトル + 背景 + 修正例 + 関連）
  - W001〜W007 の説明文
- [ ] C-2: `cmd_explain_code(code: &str)` を実装
...
- [ ] D-1: `Some("doc")` 分岐に `--builtins` フラグを追加
  - `--builtins` → `cmd_doc_builtins(format, out)`
  - それ以外 → 既存の `cmd_doc(path, out_dir)`（変更なし）
- [ ] D-2: `Some("explain")` 分岐に `cmd_explain_code` を追加
...
- [ ] E-1: `doc_builtins_json_is_array` — JSON 出力が配列であること
- [ ] E-2: `doc_builtins_csv_parse_raw` — `Csv.parse_raw` エントリが JSON に存在すること
- [ ] E-3: `doc_builtins_postgres_returns_result` — `Postgres.execute_raw` の `returns_result` が `true`
- [ ] E-4: `doc_builtins_markdown_has_namespace_header` — Markdown に `## IO` / `## Csv` が含まれる
- [ ] E-5: `explain_e0018_output` — E0018 の説明に "already bound" または "束縛" が含まれる
- [ ] E-6: `explain_w006_output` — W006 の説明に "Result" が含まれる
- [ ] E-7: `explain_unknown_returns_none` — 未知コードで `get_explain_text` が `None` を返す
- [ ] E-8: `version_is_12_7_0`
- [ ] E-9: `cargo test v12700` — 8 件通過確認

---

## Phase F — バージョン更新・コミット

- [ ] F-1: `fav/Cargo.toml` version → `"12.7.0"`
- [ ] F-2: `cargo test` — 全通過
- [ ] F-3: `git commit -m "feat: v12.7.0 — fav doc --builtins + fav explain <code>"`
